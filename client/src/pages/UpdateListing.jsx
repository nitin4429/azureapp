...
export default function CreateListing() {
    ...
    const [uploading, setUploading] = useState(false);//when upload image
    const [error, setError] = useState(false);
    const [loading, setLoading] = useState(false);//when  create listing
    const params=useParams();//hooks use to get the 'listingId' from the url /listing-update/:listingID
    ...
    const handleChange = (e) => {
        ...
        if (e.target.id === 'parking' || e.target.id === 'furnished' || e.target.id === 'offer') {
            setFormData({ ...formData, [e.target.id]: e.target.checked })//(e.id.checked give true value if we check or false value if we uncheck)
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (+formData.regularPrice < +formData.discountPrice) return setError('Discount price must be lower than regular price')//sometime formData.regularPrice is string and sometime it is number so to prevent the error we use '+' as a prefix 
            setLoading(true);
            setError(false);
